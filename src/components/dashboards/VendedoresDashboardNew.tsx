...
export default function VendedoresDashboard({ userEmail, onLogout, darkMode, onToggleDarkMode }: VendedoresDashboardProps) {
  ...
  return (
    <div className={`min-h-screen flex ${darkMode ? 'dark bg-gray-900' : 'bg-[#f7ede1]'}`}>
      ...
      {activeModal === 'clonagem' && <ClonagemMultimodalModal onClose={() => setActiveModal(null)} darkMode={darkMode} />}
      {activeModal === 'deepfake' && <DeepfakeSellerModal onClose={() => setActiveModal(null)} darkMode={darkMode} />}
    </div>
  );
}
